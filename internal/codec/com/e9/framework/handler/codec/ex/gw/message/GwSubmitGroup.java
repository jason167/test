...
/**
 * 短信群发命令，见《短信通道协议开发包V3.06(内部使用)》3.2.5.4章节
 * @date 2013-9-2
 * @author Jason
 */
public class GwSubmitGroup extends AbstractGwMessage {

	/**
	 * 
	 */
	private static final long serialVersionUID = 5663194770402801931L;
	/**
	 * 消息通道路径ID, 
	 */
	 ...
	 * 接收短信的手机号码
	 */
	private LinkedList<String> toPhone = new LinkedList<String>();
	 ...
	 * 
	 */
	public GwSubmitGroup() {
		// TODO Auto-generated constructor stub
		setCommandId(GwCommandId.SUBMIT_GROUP);
	}
	...
	public void decode(IoBuffer arg0) throws Exception {
		...
		try {
			...
			Integer toPhoneLength = readInt(buffer, GwLength.I_TARGET);
			for (int i = 1; i <= toPhoneLength; i++) {
	...
	public void encode(IoBuffer arg0) throws Exception {
		...
		try {
			...
			writeInt(buffer, getiTarget(), GwLength.I_TARGET);
			for (String to : toPhone) {
				if (to.startsWith("86")) {
					to = to.substring(2);
				}else if(to.startsWith("+86")){
					to = to.substring(3);
				}
				writeOctetString(buffer, to, GwLength.TO_PHONE);
			}
	...
	/**
	 * @param iTarget the iTarget to set
	 */
	private void setiTarget(int iTarget) {
	 ...
	 * @return the toPhone
	 */
	public LinkedList<String> getToPhone() {
		return toPhone;
	}
	...
	public void setToPhone(String toPhone) {
		this.iTarget++;
		this.toPhone.add(toPhone);
	}
	
	public void setToPhoneList(LinkedList<String> toPhoneList){
		this.toPhone.addAll(toPhoneList);
		setiTarget(this.toPhone.size());
	}
	...
	public String toString() {
		return super.toString() + "GwSubmitGroup [agentID=" + agentID + ", fromPhone=" + fromPhone + ", iTarget="
				+ iTarget + ", toPhone=" + toPhone + ", feeUser=" + feeUser + ", feePhone="
				+ feePhone + ", serviceID=" + serviceID + ", feeType=" + feeType + ", feeValue="
				+ feeValue + ", messageType=" + messageType + ", moID=" + moID + ", messageLen="
				+ messageLen + ", message=" + message + ", signLen=" + signLen + ", sign=" + sign
				+ "]";
	}
